...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const closeMenu = () => {
    ...
    // keep drawer in DOM for animation; hide after transition
  ...
  // Close on route change if using client-side nav (optional defensive)
  document.addEventListener('click', (e) => {
    if (!header.classList.contains('menu-open')) return;
    const target = e.target;
    if (target instanceof HTMLAnchorElement && target.closest('#primary-nav')) {
      closeMenu();
    }
  });
});
